...
import static com.sun.org.apache.xalan.internal.lib.ExsltDatetime.time;
import static com.sun.org.apache.xalan.internal.lib.ExsltDatetime.time;
import static houssem.sakli.NavigateurAgent.jTable8;

/**
 *
 * @author sakli
 */
public class AjoutEmployeA extends javax.swing.JFrame {
    ...
    int yMouse;
    NavigateurAgent nn;
    public AjoutEmployeA(NavigateurAgent n){
        nn=n;
        initComponents();
        seticon();
        ...
        public void ajoutemployes() throws SQLException, ClassNotFoundException
    {   
        ...
        String requete = "insert into employes(mat,nom,prenom,nomentreprise,service) values(?,?,?,?,?)";
        stmt = c.prepareStatement(requete);
        stmt.setString(1, matc);
        stmt.setString(2, nomc);
        stmt.setString(3, prenomc);
        stmt.setString(4, nomentc);
        stmt.setString(5, servicec);  
        stmt.execute();
    ...
    private void ajouterMouseClicked(java.awt.event.MouseEvent evt) {//GEN-FIRST:event_ajouterMouseClicked
        try {
            if(matriculeexist(matricule.getText())==false&&validnom(nom.getText())&&validnom(prenom.getText())&&validmatricule(matricule.getText())&&(!"Sélectionner".equals((String) jComboBoxService.getSelectedItem())))
            {
                ajoutemployes();
                JOptionPane pane = new JOptionPane("Employé ajouté avec succès !");
                JDialog d = pane.createDialog(jPanel3, "SUCCES");
                d.setLocationRelativeTo(jPanel3);
                d.setVisible(true);
                DefaultTableModel model = (DefaultTableModel) jTable8.getModel();
                model.addRow(new Object[]{matricule.getText(),nom.getText(),prenom.getText(),(String) jComboBoxService.getSelectedItem(),0});
            ...
            Logger.getLogger(AjoutEmployeA.class.getName()).log(Level.SEVERE, null, ex);
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(AjoutEmployeA.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
